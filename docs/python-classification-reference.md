...
| exact string-key dict literal `.items()` destructuring such as `for label, obj in samples.items(): label.strip()` | pure for the key variable only, and only while the mapping stays string-key-only |
| trusted direct imports of `msgraph.GraphServiceClient` MRO metadata such as `for cls in GraphServiceClient.__mro__: cls.__module__.startswith('msgraph')` | pure |
| exact direct imports of `oci.container_engine.models.CreateClusterDetails` metadata maps such as `CreateClusterDetails.swagger_types.get(...)` and `CreateClusterDetails.attribute_map.get(...)` | pure |
...
| exact unaliased direct imports of `pytfe.models.organization_membership.OrganizationMembershipListOptions`, followed by `.model_validate(...)` and direct local `.model_dump(...)` on the validated instance | pure for `.model_dump(...)` only; the validating call remains conservative |
...
| tracked sqlite connection `.cursor()` after `conn = sqlite3.connect(...)` | exec |
| tracked sqlite cursor `.execute(...)` and `.close()` after `conn = sqlite3.connect(...); cur = conn.cursor()` | exec |
| tracked sqlite cursor `.fetchone()` / `.fetchall()` after the same cursor has already executed a query | read |
